...
app.get('/api/notification-preferences', auth, (req, res) => {
  const prefs = dbGet('SELECT * FROM notification_preferences WHERE user_id = ?', req.user.id);
  if (!prefs) {
    // Return defaults (all on)
    res.json({
      yard_duty_reminders: 1, swap_requests: 1, absence_notifications: 1,
      announcement_alerts: 1, wellbeing_reminders: 1, crt_assignments: 1, timetable_changes: 1
    });
  } else {
    res.json(prefs);
  }
...
app.put('/api/notification-preferences', auth, (req, res) => {
  const fields = ['yard_duty_reminders', 'swap_requests', 'absence_notifications',
    'announcement_alerts', 'wellbeing_reminders', 'crt_assignments', 'timetable_changes'];
  const vals = fields.map(f => req.body[f] ? 1 : 0);

  const existing = dbGet('SELECT id FROM notification_preferences WHERE user_id = ?', req.user.id);
  if (existing) {
    dbRun(`UPDATE notification_preferences SET
      yard_duty_reminders = ?, swap_requests = ?, absence_notifications = ?,
      announcement_alerts = ?, wellbeing_reminders = ?, crt_assignments = ?,
      timetable_changes = ?, updated_at = datetime('now')
      WHERE user_id = ?`, [...vals, req.user.id]);
  } else {
    dbRun(`INSERT INTO notification_preferences (user_id, yard_duty_reminders, swap_requests,
      absence_notifications, announcement_alerts, wellbeing_reminders, crt_assignments, timetable_changes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [req.user.id, ...vals]);
  }
  res.json({ ok: true });
});

// Leader: get ALL staff notification preferences
app.get('/api/notification-preferences/all', auth, leaderOnly, (req, res) => {
  const users = dbAll("SELECT id, name, role, area FROM users WHERE active = 1");
  const fields = ['yard_duty_reminders', 'swap_requests', 'absence_notifications',
    'announcement_alerts', 'wellbeing_reminders', 'crt_assignments', 'timetable_changes'];
  const result = users.map(u => {
    const prefs = dbGet('SELECT * FROM notification_preferences WHERE user_id = ?', u.id);
    const p = {};
    fields.forEach(f => { p[f] = prefs ? (prefs[f] ?? 1) : 1; });
    return { ...u, prefs: p };
  });
  res.json(result);
});
...
app.put('/api/notification-preferences/:userId', auth, leaderOnly, (req, res) => {
  const targetUserId = parseInt(req.params.userId);
  const targetUser = dbGet('SELECT id FROM users WHERE id = ? AND active = 1', targetUserId);
  ...
  const existing = dbGet('SELECT id FROM notification_preferences WHERE user_id = ?', targetUserId);
  if (existing) {
    dbRun(`UPDATE notification_preferences SET
      yard_duty_reminders = ?, swap_requests = ?, absence_notifications = ?,
      announcement_alerts = ?, wellbeing_reminders = ?, crt_assignments = ?,
      timetable_changes = ?, updated_at = datetime('now')
      WHERE user_id = ?`, [...vals, targetUserId]);
  } else {
    dbRun(`INSERT INTO notification_preferences (user_id, yard_duty_reminders, swap_requests,
      absence_notifications, announcement_alerts, wellbeing_reminders, crt_assignments, timetable_changes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [targetUserId, ...vals]);
  }
  res.json({ ok: true });
});
...
async function start() {
  ...
  try {
    db.run(`CREATE TABLE IF NOT EXISTS notification_preferences (
      ...
      yard_duty_reminders INTEGER DEFAULT 1,
      swap_requests INTEGER DEFAULT 1,
      absence_notifications INTEGER DEFAULT 1,
      announcement_alerts INTEGER DEFAULT 1,
      wellbeing_reminders INTEGER DEFAULT 1,
      crt_assignments INTEGER DEFAULT 1,
      timetable_changes INTEGER DEFAULT 1,
